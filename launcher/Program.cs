...
if (missing.Count > 0)
{
    ...
    Console.WriteLine();
    Console.WriteLine("Run these commands in PowerShell (as Administrator):");
    Console.WriteLine("  $url = \"https://raw.githubusercontent.com/jkudo/local_llm_manager/refs/heads/master/setup-runtime-files.ps1\"");
    Console.WriteLine("  Invoke-WebRequest -Uri $url -OutFile $env:TEMP\\setup-runtime-files.ps1");
    Console.WriteLine("  & $env:TEMP\\setup-runtime-files.ps1");
    Console.WriteLine();
    Console.WriteLine("Note: Right-click PowerShell and choose 'Run as administrator'");
    Console.WriteLine("      (required to write to C:\\Program Files)");
    Console.WriteLine();
    Console.WriteLine("If you installed to a custom folder:");
    Console.WriteLine("  & $env:TEMP\\setup-runtime-files.ps1 -InstallDir \"C:\\Path\\To\\Local LLM Manager\"");
    Console.WriteLine();
